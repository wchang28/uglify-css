...
exports.processString = exports.defaultOptions = void 0;
var _a = require('path'), sep = _a.sep, resolve = _a.resolve;
/**
...
function convertRelativeUrls(css, options, preservedTokens) {
    ...
    while ((match = pattern.exec(css)) !== null) {
        var startIndex = match.index + match[1].length; // 'url('.length()
        var terminator = match[2]; // ', " or empty (not quoted)
        if (terminator.length === 0) {
            terminator = ')';
        }
        var foundTerminator = false;
        var endIndex = pattern.lastIndex - 1;
        ...
        if (foundTerminator) {
            var token = css.substring(startIndex, endIndex).replace(/(^\s*|\s*$)/g, '');
            if (token.slice(0, 19) !== ___PRESERVED_TOKEN_) {
                ...
                if (options.convertUrls && token.charAt(0) !== SEP && token.slice(0, 7) !== 'http://' && token.slice(0, 8) !== 'https://') {
                    ...
                    var target = options.target.slice();
                    ...
                    url = terminator + __spread(target, url, [file]).join(SEP) + terminator;
                }
                else {
                    url = terminator + token + terminator;
                }
                preservedTokens.push(url);
                var preserver = 'url(' + ___PRESERVED_TOKEN_ + (preservedTokens.length - 1) + '___)';
                sb.push(preserver);
            }
            else {
                sb.push("url(" + token + ")");
...
function extractDataUrls(css, preservedTokens) {
    ...
    // handling sb appends directly, instead of using matcher.append* methods.
    while ((match = pattern.exec(css)) !== null) {
        var startIndex = match.index + 4; // 'url('.length()
        var terminator = match[1]; // ', " or empty (not quoted)
        if (terminator.length === 0) {
            terminator = ')';
        }
        var foundTerminator = false;
...
function keyframes(content, preservedTokens) {
    var pattern = /@[a-z0-9-_]*keyframes\s+[a-z0-9-_]+\s*{/gi;
    var index = 0;
    var buffer;
    var preserve = function (part, i) {
    ...
    while (true) {
        var level = 0;
        buffer = '';
        var startIndex = content.slice(index).search(pattern);
        if (startIndex < 0) {
            break;
        }
        index += startIndex;
...
function collectComments(content, comments) {
    ...
    var end;
    while (true) {
        var start = content.indexOf('/*', from);
        if (start > -1) {
...
function processString(content, options) {
    ...
    for (var i = 0, len = comments.length; i < len; i += 1) {
        ...
        var placeholder = '___PRESERVE_CANDIDATE_COMMENT_' + i + '___';
        ...
        // e.g. html >/**/ body
        if (token.length === 0) {
            var startIndex = content.indexOf(placeholder);
            if (startIndex > 2) {
    ...
    if (options.expandVars) {
        ...
        content = content.replace(pattern, function (_, f1) {
            pattern = /\s*([a-z0-9\-]+)\s*:\s*([^;\}]+)\s*/gi;
            f1.replace(pattern, function (_, f1, f2) {
                if (f1 && f2) {
                    vars_1[f1] = f2;
                }
                return '';
    ...
    pattern = /(^|\})(([^\{:])+:)+([^\{]*\{)/g;
    content = content.replace(pattern, function (token) { return token.replace(/:/g, '___PSEUDOCLASSCOLON___'); });
    ...
    content = content.replace(pattern, function (_, f1, f2) {
        f2 = f2.replace(/(^|\D)0?\.?0(m?s)/gi, function (_, g1, g2) {
    ...
    content = content.replace(/(hsla?)\(([^)]+)\)/g, function (_, f1, f2) {
        var f0 = [];
    ...
    content = content.replace(/:first-(line|letter)(\{|,)/gi, function (_, f1, f2) { return ':first-' + f1.toLowerCase() + ' ' + f2; });
    ...
    pattern = /^(.*)(@charset)( "[^"]*";)/gi;
    content = content.replace(pattern, function (_, f1, f2, f3) { return f2.toLowerCase() + f3 + f1; });
    // When all @charset are at the top, remove the second and after (as they are completely ignored).
    pattern = /^((\s*)(@charset)( [^;]+;\s*))+/gi;
    content = content.replace(pattern, function (_, __, f2, f3, f4) { return f2 + f3.toLowerCase() + f4; });
    // lowercase some popular @directives (@charset is done right above)
    pattern = /@(font-face|import|(?:-(?:atsc|khtml|moz|ms|o|wap|webkit)-)?keyframe|media|page|namespace)/gi;
    content = content.replace(pattern, function (_, f1) { return '@' + f1.toLowerCase(); });
    ...
    content = content.replace(pattern, function (_, f1, f2) { return f1 + f2.toLowerCase(); });
    ...
    content = content.replace(pattern, function (_, f1, f2) { return f1.toLowerCase() + ':0 0' + f2; });
    ...
    content = content.replace(pattern, function (_, f1) {
        var rgbcolors = f1.split(',');
        var hexcolor = '#';
        for (var i = 0; i < rgbcolors.length; i += 1) {
            var val = parseInt(rgbcolors[i], 10);
            if (val < 16) {
                hexcolor += '0';
            }
            if (val > 255) {
    ...
    content = content.replace(pattern, function (_, f1, f2) { return f1.toLowerCase() + ':0' + f2; });
    ...
    for (var i = preservedTokens.length - 1; i >= 0; i--) {
